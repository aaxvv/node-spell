...
// Catch situations like this, where data from a flow node is used before it is executed
//
//          +---------+     +---------+
// FLOW --> | Set Var | --> | Get Var | --> FLOW
//       +->O         |     |         O--+
//       |  +---------+     +---------+  |
//       |                               |
//       +---------------<---------------+
public class DataUsedBeforeFlowReachedRule implements VerificationRule {
    @Override
    public void check(SpellGraph graph, Consumer<VerificationResult> resultConsumer) {
        Set<NodeInstance> incorrectFlowNodes = new HashSet<>();
        ...
        recursiveDfs(entrypoint, dfsStack, true, incorrectFlowNodes);

        if (!incorrectFlowNodes.isEmpty()) {
            resultConsumer.accept(VerificationResult.error(Component.translatable("gui.node_spell.spell_verification.incorrect_flow_order"), incorrectFlowNodes.stream().toList()));
        }
    }
            ...
            if (traversingFlow) {
                if (socket.getBase().getDirection().isOut() && socket.getBase().getDataType() == Datatype.FLOW) {
                    ...
                    for (Edge connected : socket.getConnections()) {
                        recursiveDfs(connected.getOpposite(socket).getParentInstance(), dfsStack, true, incorrectFlowNodes);
                    }
                } else if (socket.getBase().getDirection().isIn() && socket.getBase().getDataType() != Datatype.FLOW) {
                    // begin traversing backwards in data flow
                    for (Edge connected : socket.getConnections()) {
                        recursiveDfs(connected.getOpposite(socket).getParentInstance(), dfsStack, false, incorrectFlowNodes);
                    }
                }
            } else {
